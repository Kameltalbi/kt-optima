 ...
 * @param type - Type de document: 'acompte', 'facture', ou 'avoir'
 * @param date - Date du document (optionnel, par défaut aujourd'hui)
 * @returns Le numéro formaté (ex: AC-2026-01-001, F-2026-01-002, AV-2026-01-003)
 */
export async function getNextDocumentNumber(
  type: 'acompte' | 'facture' | 'avoir',
  date?: string
): Promise<string> {
  try {
    ...
    const prefix = type === 'acompte' ? 'AC' : type === 'avoir' ? 'AV' : 'F';
